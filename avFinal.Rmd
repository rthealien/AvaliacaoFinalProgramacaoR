...
dadosMapa <- structure(
  list(
    ...
    valor = scales::rescale(gatos_estado$valor)
  ),
  class = "data.frame",
  row.names = c(NA,-27L)
)
...
p = states %>% ggplot() +
  geom_sf(aes(fill =  valor), size = 0.15) +
  scale_fill_gradient(
    low = "red",
    high = "blue",
    ...
    limits = c(L, S),
    #breaks=c(L,S),
    #labels=c("Menor Valor","Maior Valor")
    
  ) +
  xlab("") +  ylab("") + geom_sf_label(aes(label = abbrev_state),
...
gastos_partidos = dadosDeputados %>% 
                    group_by(sgPartido) %>% 
                    summarise(valor = sum(as.numeric(vlrLiquido), na.rm = TRUE),
                              valor_por_qtd_deputado = sum(as.numeric(vlrLiquido), na.rm = TRUE)/n_distinct(cpf)) %>% 
                    arrange(desc(valor))
